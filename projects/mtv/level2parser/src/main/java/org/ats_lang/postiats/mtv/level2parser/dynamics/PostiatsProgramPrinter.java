...
public class PostiatsProgramPrinter {

    private STGroup m_stg;
    private STGroup m_stg_type;
    
    public  PostiatsProgramPrinter() {
    ...
    public String print(PostiatsProgram uProg) {
        ST st = printUtfplProgram(uProg);
        return st.render();
    }
    
    private ST printUtfplProgram(PostiatsProgram uProg) {
        // utfpl_prog_st(d2ecs) ::= <<
    ...
    private ST printId2ecl_node(Id2ecl_node node) {
        ...
        } else if (node instanceof D2Clocal) {
        	return printD2Clocal((D2Clocal)node);            
        } else {
            throw new Error("Id2ecl_node " + node + " is not supported");
        }
    }
    
    private ST printD2Clocal(D2Clocal node) {
        // D2Clocal(d2cs0, d2cs1) ::= <<
        ST st = m_stg.getInstanceOf("D2Clocal");
        for (Cd2ecl dec: node.m_d2cs) {
        	st.add("d2cs1", printCd2ecl(dec));
        }
        return st;
	}
